...
import cn.edu.nju.software.tongbaoshipper.controller.adapter.AutoAddressAdapter;

/**
 * 演示poi搜索功能
 */
public class AddressFromActivity extends AppCompatActivity implements
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_address_from_search);
        initView();
        // 初始化搜索模块，注册搜索事件监听
    ...
    private void initView()
    {
        ...
        nameinput =(EditText) findViewById(R.id.address_from_name);
        phoneinput =(EditText) findViewById(R.id.address_from_phone);
        back_btn =(LinearLayout) findViewById(R.id.address_from_btn_back);
        ok_btn  =(LinearLayout) findViewById(R.id.address_from_btn_ok);
    ...
    public void onGetPoiResult(PoiResult result) {
        if (result == null
            ...
            Toast.makeText(AddressFromActivity.this, "未找到结果", Toast.LENGTH_LONG)
                    .show();
            return;
        }
        if (result.error == SearchResult.ERRORNO.NO_ERROR) {
            ...
            for (PoiInfo info : result.getAllPoi()) {
                if (info.location!=null&&info.type.name().equals("POINT"))  addressList.add(info);
                System.out.println(info.name + "***" + info.address+"***"+info.type.name());
            }
        ...
        if (result.error == SearchResult.ERRORNO.AMBIGUOUS_KEYWORD) {
            ...
            strInfo += "找到结果";
            Toast.makeText(AddressFromActivity.this, strInfo, Toast.LENGTH_LONG)
                    .show();
        }
    }
    ...
    public void onGetPoiDetailResult(PoiDetailResult result) {
        if (result.error != SearchResult.ERRORNO.NO_ERROR) {
            Toast.makeText(AddressFromActivity.this, "抱歉，未找到结果", Toast.LENGTH_SHORT)
                    .show();
        } else {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.address_from_btn_back:
                Log.d(AddressFromActivity.class.getName(), "back");
                finish();
                break;
            case R.id.address_from_btn_ok:
                Log.d(AddressFromActivity.class.getName(), "ok");
                System.out.println(phoneinput.length());
                if (longitude == 1024){
                    Toast.makeText(AddressFromActivity.this,"请输入并选择详细地址",Toast.LENGTH_SHORT).show();
                } else if (keyWorldsView.length()*nameinput.length()*phoneinput.length()!=0) {
                    Intent intent = new Intent();
                    intent.putExtra("from", keyWorldsView.getText().toString());
                    intent.putExtra("name", nameinput.getText().toString());
                    intent.putExtra("phone", phoneinput.getText().toString());
                    intent.putExtra("longitude",longitude);
                    intent.putExtra("latitude",latitude);
                    setResult(RESULT_CODE_START, intent);
                    finish();
                }
                else {
                    Toast.makeText(AddressFromActivity.this, "输入不可为空", Toast.LENGTH_LONG)
                            .show();
                }
                break;
            default:
                Log.e(AddressFromActivity.class.getName(), "Unknown id");
        }
    }
